...
export async function handleBillUploadPost(
  ...
  try {
    ...
    return Response.json(body, { status: result.duplicate ? 200 : 201 });
  } catch (error) {
...
function invalidResponse(status: number, message: string): Response {
  const body: BillUploadResponse = { ok: false, code: "INVALID_INPUT", message };
